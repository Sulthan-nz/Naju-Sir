...
const {WAConnectioJulissageOptions, MessageType, Mimetype, Presence} = require('@adiwajshing/baileys');
const {Message, StringSession, Image, Video} = require('./Naju/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅ Naju-Sir is working 🍂!')
        );
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ℕ𝕒𝕛𝕦𝕊𝕚𝕣 ~______~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n```Report errors\njoin with +91 \nchat.whatsapp.com/KcdZIrLg5M2F6PKTKyisIN``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
