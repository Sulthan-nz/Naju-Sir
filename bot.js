...
const {Message, StringSession, Image, Video} = require('./NAJU-SIR/');
const { DataTypes } = require('sequelize');
const { GreetingsDB, getMessage } = require("./plugins/sql/greetings");
const got = require('got');
const NAJU-SIRStack = require('NAJU-SIR-npm');
...
// Sql
const NAJU-SIRDB = config.DATABASE.define('NAJU-SIRDuplicated', {
    info: {
...
async function NAJU-SIR () {
    ...
    clh.exc_pl = exc_fn
    clh.pay = ddd
    const NAJU-SIRCN = new WAConnection();
    const Session = new StringSession();
    NAJU-SIRCN.version = [2, 2126, 14]
    NAJU-SIRCN.setMaxListeners(0);
    var proxyAgent_var = ''
    if (config.PROXY.includes('https') || config.PROXY.includes('http')) {
      NAJU-SIRCN.connectOptions.agent = ProxyAgent (config.PROXY)
    }
    setInterval(async () => { 
        var getGMTh = new Date().getHours()
        var getGMTm = new Date().getMinutes()
        var ann_msg = await NAJU-SIRStack.daily_announcement(config.LANG)
        var ann = await NAJU-SIRStack.ann()
        while (getGMTh == 19 && getGMTm == 1) {
            ...
            if (ann.video.includes('http') || ann.video.includes('https')) {
                ...
                yt.on('end', async () => {
                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video, {caption: ilan + ann_msg.replace('{user}', NAJU-SIRCN.user.name).replace('{wa_version}', NAJU-SIRCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', NAJU-SIRCN.user.phone.os_version).replace('{device_model}', NAJU-SIRCN.user.phone.device_model).replace('{device_brand}', NAJU-SIRCN.user.phone.device_manufacturer), mimetype: Mimetype.mp4});
                });
            } else {
                if (ann.image.includes('http') || ann.image.includes('https')) {
                    var imagegen = await axios.get(ann.image, { responseType: 'arraybuffer'})
                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, Buffer.from(imagegen.data), MessageType.image, { caption: ilan + ann_msg.replace('{user}', NAJU-SIRCN.user.name).replace('{wa_version}', NAJU-SIRCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', NAJU-SIRCN.user.phone.os_version).replace('{device_model}', NAJU-SIRCN.user.phone.device_model).replace('{device_brand}', NAJU-SIRCN.user.phone.device_manufacturer)})
                } else {
                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, ilan + ann_msg.replace('{user}', NAJU-SIRCN.user.name).replace('{wa_version}', NAJU-SIRCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', NAJU-SIRCN.user.phone.os_version).replace('{device_model}', NAJU-SIRCN.user.phone.device_model).replace('{device_brand}', NAJU-SIRCN.user.phone.device_manufacturer), MessageType.text)
                }
            }
        }
    }, 50000);
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            var timezone_bio = await NAJU-SIRStack.timezone(NAJU-SIRCN.user.jid)
            var date_bio = await NAJU-SIRStack.datebio(config.LANG)
            const biography = '📅 ' + date_bio + '\n⌚ ' + timezone_bio
            await NAJU-SIRCN.setStatus(biography)
        }
    }, 7890);
    var shs1 = ''
    var shl2 = ''
    ...
    var StrSes_Db = await NAJU-SIRDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    NAJU-SIRCN.logger.level = logger_levels
    var nodb;
    if (StrSes_Db.length < 1) {
        nodb = true;
        NAJU-SIRCN.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        NAJU-SIRCN.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }
    NAJU-SIRCN.on('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login Information Updated!')
        );
        const authInfo = NAJU-SIRCN.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
            await NAJU-SIRDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
            await StrSes_Db[0].update({ value: Session.createStringSession(authInfo) });
        }
    })    
    NAJU-SIRCN.on('connecting', async () => {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    NAJU-SIRCN.on('open', async () => {
        ...
        console.log(chalk.bgGreen('🐺 NAJU-SIR' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*NAJU-SIR Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*NAJU-SIR Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*NAJU-SIR Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using NAJU-SIR 💌*\n    *- Eva*'
        if (NAJU-SIRCN.user.jid == one || NAJU-SIRCN.user.jid == two || NAJU-SIRCN.user.jid == three || NAJU-SIRCN.user.jid == four || NAJU-SIRCN.user.jid == five || NAJU-SIRCN.user.jid == six || NAJU-SIRCN.user.jid == seven || NAJU-SIRCN.user.jid == eight) {
            await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid,nw, MessageType.text), console.log(nw), await new Promise(r => setTimeout(r, 1000))
            await heroku.get(baseURI + '/formation').then(async (formation) => { 
                forID = formation[0].id; 
                await heroku.patch(baseURI + '/formation/' + forID, { 
                    body: { 
        ...
        if (config.FULLEVA == 'true') {
            var eva_msg = await NAJU-SIRStack.eva_if(config.LANG)
            await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, eva_msg, MessageType.text)
        }
        else {
            var af_start = await NAJU-SIRStack.work_type(config.WORKTYPE, config.LANG)
            await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, af_start, MessageType.text)
        }
        await git.fetch();
        var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
        if (commits.total === 0) {
            await NAJU-SIRCN.sendMessage(
                NAJU-SIRCN.user.jid,
            ...
            var up_ch = await NAJU-SIRStack.update(config.LANG)
            await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, up_ch, MessageType.text)
        }
    })
    NAJU-SIRCN.on("chat-update", async (m) => {
        ...
        if (config.NO_ONLINE) {
            await NAJU-SIRCN.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        // ==================== Greetings ====================
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                if (gb.message.includes('{gpp}')) {
                    var ppUrl = await NAJU-SIRCN.getProfilePicture(msg.key.remoteJid) 
                    var nwjson = await NAJU-SIRCN.groupMetadata(msg.key.remoteJid)
                    const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
                    await NAJU-SIRCN.sendMessage(msg.key.remoteJid, Buffer.from(resim.data), MessageType.image, { mimetype: Mimetype.png, caption: gb.message.replace('{gpp}', '').replace('{botowner}', NAJU-SIRCN.user.name).replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc) });
                } else {
                    var nwjson = await NAJU-SIRCN.groupMetadata(msg.key.remoteJid)
                    await NAJU-SIRCN.sendMessage(msg.key.remoteJid, gb.message.replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc).replace('{botowner}', NAJU-SIRCN.user.name), MessageType.text);
                }
            }
            return;
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = NAJU-SIRCN.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await NAJU-SIRCN.chatRead(msg.key.remoteJid);
                        }
                        var match = text_msg.match(command.pattern);
                        if (command.on !== undefined && (command.on === 'image' || command.on === 'photo' )
                        && msg.message.imageMessage !== null) {
                            whats = new Image(NAJU-SIRCN, msg);
                        } else if (command.on !== undefined && (command.on === 'video')
                        && msg.message.videoMessage !== null) {
                            whats = new Video(NAJU-SIRCN, msg);
                        } else {
                            whats = new Message(NAJU-SIRCN, msg);
                        }
                        ...
                        try {
                            ...
                            var error_report = await NAJU-SIRStack.error(config.LANG)
                            await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, error_report.replace('{real_error}', error), MessageType.text, {detectLinks: false})

                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, '*⚕️ HATA ÇÖZÜMLEME [NAJU-SIR] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                    ...
                                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, '*⚕️ ERROR ANALYSIS [NAJU-SIR] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await NAJU-SIRCN.sendMessage(NAJU-SIRCN.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
